# GenRx Documentation

> **Type:** Documentation Index
> **Last updated:** 2026-05-30

## Table of Contents
- [Core Technical References](#core-technical-references)
- [Procedural Guides](#procedural-guides)
- [User Guides](#user-guides)
- [Supplementary Documents](#supplementary-documents)
- [Meta](#meta)

## Core Technical References
- [Architecture Deep Dive](./ARCHITECTURE_DEEP_DIVE.md) — System architecture, database schema, entity relationships
- [Dependency Management](./DEPENDENCY_MANAGEMENT.md) — Python and Node.js dependency strategy
- [Data Ingestion](./DATA_INGESTION.md) — Document processing pipeline and vector store
- [Bedrock Guardrails](./BEDROCK_GUARDRAILS.md) — AI content filtering and prompt injection defenses

## Procedural Guides
- [Deployment Guide](./DEPLOYMENT_GUIDE.md) — Full deployment from scratch
- [Modification Guide](./MODIFICATION_GUIDE.md) — Customizing colors, API, LLM, and frontend
- [Database Migrations](./DATABASE_MIGRATIONS.md) — Creating and running schema changes

## User Guides
- [User Guide](./USER_GUIDE.md) — Student, instructor, and admin workflows

## Supplementary Documents
- [CDK Technical Review](./CDK_TECHNICAL_REVIEW.md) — Infrastructure code review and recommendations
- [Security Overview](./SECURITY_OVERVIEW.md) — OWASP assessment and remediation roadmap
- [Voice Agent Deep Dive](./VOICE_AGENT_DEEP_DIVE.md) — Voice architecture and design decisions
- [AgentCore Voice Agent Setup](./AGENTCORE_VOICE_AGENT_SETUP.md) — Console-side voice agent configuration

## Meta
- [Changelog](./CHANGELOG.md) — Notable changes log
- [Contributing to Docs](./CONTRIBUTING_DOCS.md) — Documentation maintenance rules

