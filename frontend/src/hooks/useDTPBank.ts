...
/**
 * Pagination state for a DTP bank tab
 */
...
/**
 * Parameters for the useDTPBank hook
 */
export interface UseDTPBankParams {
  /** Role determines loading strategy and UX flow */
  role: 'admin' | 'instructor';
}

/**
 * Return type for the useDTPBank hook
 */
export interface UseDTPBankReturn {
  // Item lists (raw, unfiltered)
  dtpItems: DTPItem[];
  setDtpItems: React.Dispatch<React.SetStateAction<DTPItem[]>>;

  // Filtered item list
  filteredItems: DTPItem[];
  ...
  getPaginatedItems: (items: DTPItem[], currentPage: number, itemsPerPage: number) => DTPItem[];
 ...
 * useDTPBank hook
 *
 * Manages DTP bank state including search, pagination, included/pending IDs,
 ...
 */
export function useDTPBank({ role: _role }: UseDTPBankParams): UseDTPBankReturn {
  // Item lists
  ...
  const filteredItems = useMemo(() => {
    if (!searchQuery) return dtpItems;
    return dtpItems.filter(item =>
      item.title.toLowerCase().includes(searchQuery.toLowerCase())
    );
  }, [dtpItems, searchQuery]);

  // Pagination helpers
  const getPaginatedItems = (items: DTPItem[], currentPage: number, itemsPerPage: number) => {
