...
export interface DTPBankSectionProps {
  ...
  onToggleDTPInclusion?: (dtpId: string, dtpItem: DTPItem, isChecked: boolean) => void;
 ...
 *
 * Renders the DTP bank UI with group-wide and patient-specific tabs,
        ...
        <div className="space-y-3">
          ...
          {activeTab === 'patientSpecific' && (
            <>
              <p className="text-sm mb-4" style={{ color: UI_COLORS.text.muted }}>
                {role === 'admin'
                  ? "Select a patient to manage their patient-specific DTP items."
                  : "Select a patient to manage their patient-specific DTP items. These apply only to the selected patient encounter."}
              </p>
              ...
              {selectedPatientId ? (
                ...
                <p className="text-sm text-center py-8" style={{ color: UI_COLORS.text.muted }}>
                  Please select a patient to manage their DTP items.
                </p>
              )}
            </>
          )}
...
/**
 * Accordion-style DTP item used in the instructor flow.
 */
function DTPAccordionItem({
  item,
  isChecked,
  onToggle,
}: {
      ...
      <AccordionContent className="px-4 pb-4" style={{ backgroundColor: UI_COLORS.background.white }}>
        <div className="space-y-3 pt-3">
          ...
          <div>
            ...
            <p className="text-sm whitespace-pre-line" style={{ color: item.expectedDTPText ? UI_COLORS.text.body : UI_COLORS.text.muted }}>{item.expectedDTPText || '—'}</p>
          </div>
          <div>
            <label className="block text-xs font-semibold mb-1" style={{ color: UI_COLORS.text.muted }}>Clinical Intent</label>
            <p className="text-sm whitespace-pre-line" style={{ color: item.clinicalIntent ? UI_COLORS.text.body : UI_COLORS.text.muted }}>{item.clinicalIntent || '—'}</p>
          </div>
          <div>
